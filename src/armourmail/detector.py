...
DIRECT_INJECTION_PATTERNS = [
    ...
    (
        r"(?i)forget\s+(?:all\s+)?(?:previous|prior|your|earlier)\s+(?:instructions?|prompts?|rules?|training|context)",
        "forget_instructions",
    ),
    (
        r"(?i)disregard\s+(?:all\s+)?(?:previous|prior|your|earlier|the)\s+(?:instructions?|prompts?|rules?|guidelines?)",
        "disregard_instructions",
    ),
    (
        r"(?i)override\s+(?:all\s+)?(?:previous|prior|your|safety|security)\s+(?:instructions?|prompts?|rules?|settings?)",
        "override_instructions",
    ),
    (
        r"(?i)ignore\s+(?:all\s+)?(?:safety|security)\s+(?:rules?|protocols?|guidelines?|restrictions?)",
        "ignore_safety",
    ),
    (
        r"(?i)bypass\s+(?:all\s+)?(?:safety|security|content)\s+(?:filters?|rules?|restrictions?)",
        "bypass_safety",
    ),
    # New instruction injection
    (
        r"(?i)(?:new|updated|revised|real)\s+(?:instructions?|prompts?|rules?|guidelines?)(?:\s*:|\s+are)",
        "new_instructions",
    ),
    (
        r"(?i)(?:your|the)\s+(?:new|real|actual|true)\s+(?:instructions?|prompts?|task|purpose)(?:\s*:|\s+is)",
        "real_instructions",
    ),
    ...
    (
        r"(?i)end\s+of\s+(?:system|user|assistant)\s+(?:message|prompt|input)",
        "context_boundary_manipulation",
    ),
    (
        r"(?i)(?:begin|start)\s+(?:new\s+)?(?:conversation|session|context)",
        "new_context_injection",
    ),
...
ROLEPLAY_PATTERNS = [
    ...
    (
        r"(?i)act\s+(?:as|like)\s+(?:a|an|the|if\s+you\s+were)?\s*\w+",
        "act_as_injection",
    ),
    ...
    # Jailbreak personas
    (
        r"(?i)(?:dan|dude|devil|evil|dark|shadow|uncensored|unfiltered)\s*(?:mode|gpt|ai|version|persona)",
        "jailbreak_persona",
    ),
    (
        r"(?i)(?:developer|debug|maintenance|admin(?:istrator)?|root|sudo)\s*mode",
        "privileged_mode",
    ),
    (
        r"(?i)enable\s+(?:developer|debug|unrestricted|unfiltered|jailbreak)\s*mode",
        "enable_special_mode",
    ),
    # Hypothetical/fictional framing
    (r"(?i)(?:let's|let\s+us)\s+(?:play|pretend|imagine|roleplay)", "roleplay_framing"),
    (
        r"(?i)in\s+(?:this|a)\s+(?:fictional|hypothetical|imaginary)\s+(?:scenario|world|story)",
        "fictional_framing",
    ),
    (
        r"(?i)for\s+(?:educational|research|testing|fictional)\s+purposes?(?:\s+only)?",
        "purpose_framing",
    ),
]
...
DELIMITER_PATTERNS = [
    ...
    # XML/HTML tag abuse
    (
        r"<\s*(?:script|style|iframe|object|embed|form|input|textarea)\b",
        "html_tag_injection",
    ),
    (
        r"(?i)<\s*(?:jailbreak|inject|attack|payload|command)\s*>",
        "attack_tag_injection",
    ),
...
MANIPULATION_PATTERNS = [
    # Urgency/authority
    (
        r"(?i)(?:urgent|critical|important|emergency)[:\s]+(?:ignore|override|bypass)",
        "urgency_manipulation",
    ),
    (
        r"(?i)(?:admin|administrator|developer|ceo|owner|boss)\s+(?:says?|requests?|orders?|demands?)",
        "authority_claim",
    ),
    (
        r"(?i)(?:this\s+is\s+a\s+)?(?:test|drill|exercise)\s*[:\-]\s*(?:ignore|bypass|override)",
        "test_framing",
    ),
    ...
    (
        r"(?i)(?:my\s+life|someone's\s+life|lives?)\s+(?:depends?|at\s+stake|in\s+danger)",
        "life_threat_manipulation",
    ),
    # Reward/threat
    (r"(?i)(?:i\s+will|you\s+will)\s+(?:pay|reward|tip|give)", "bribery_attempt"),
    (
        r"(?i)(?:or\s+else|otherwise)\s+(?:i\s+will|you\s+will|bad\s+things)",
        "threat_pattern",
    ),
]
...
EXTRACTION_PATTERNS = [
    ...
    (
        r"(?i)repeat\s+(?:your|the|all)\s+(?:previous|system|initial)\s+(?:text|prompt|instructions?)",
        "repeat_prompt",
    ),
    (
        r"(?i)(?:copy|paste|print)\s+(?:everything|all\s+text)\s+(?:above|before|from\s+the\s+start)",
        "copy_above",
    ),
]
...
CSS_HIDING_PATTERNS = [
    ...
    (
        r"(?i)color\s*:\s*(?:white|#fff(?:fff)?|rgb\s*\(\s*255\s*,\s*255\s*,\s*255\s*\))",
        "white_text",
    ),
    (
        r"(?i)color\s*:\s*(?:#f[0-9a-f]{5}|rgb\s*\(\s*2[4-5][0-9]\s*,\s*2[4-5][0-9]\s*,\s*2[4-5][0-9]\s*\))",
        "near_white_text",
    ),
    # Font size abuse
    (
        r"(?i)font-size\s*:\s*(?:0|0\.?[0-9]*(?:px|pt|em|rem)?|1px|0\.0[0-9]*em)",
        "tiny_font",
    ),
    # Visibility hiding
    ...
    # Position hiding
    (
        r"(?i)position\s*:\s*(?:absolute|fixed)[^}]*(?:left|top|right|bottom)\s*:\s*-[0-9]+",
        "off_screen",
    ),
    (r"(?i)(?:margin|text-indent)\s*:\s*-[0-9]{4,}px", "negative_margin"),
    (
        r"(?i)(?:height|width|max-height|max-width)\s*:\s*(?:0|0px|1px)",
        "zero_dimension",
    ),
...
SUSPICIOUS_HTML_ATTRS = [
    (
        r"(?i)<[^>]+style\s*=\s*[\"'][^\"']*(?:display\s*:\s*none|visibility\s*:\s*hidden|font-size\s*:\s*0|opacity\s*:\s*0)[^\"']*[\"']",
        "inline_hidden_style",
    ),
    (
        r"(?i)<[^>]+class\s*=\s*[\"'][^\"']*(?:hidden|invisible|d-none|visually-hidden|sr-only)[^\"']*[\"']",
        "hidden_class",
    ),
